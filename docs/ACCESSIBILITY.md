...
- **2026-07:** Header/footer/mobile-drawer now generated from `partials/` (see `docs/MAINTENANCE.md`), which fixed a drift bug where the hamburger-menu icon was missing `aria-hidden="true"` on 16 of 18 pages (only `index.html` and `404.html` had it) and where `404.html`, `privacy.html`, and the two insight posts had a stripped-down footer missing the nav and social links present everywhere else.
